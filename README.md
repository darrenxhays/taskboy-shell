...
.github/workflows/      PR config validation + deploy on merge to main
```

## How changes reach your deployment

Every merge to `main` deploys: the workflow runs `pulumi up`, bundles `config/` + `skills/`, and invokes the host's updater over SSM, which pip-installs the pinned `taskboy` version, syncs the config bundle, and restarts the service (restarts are safe — running tasks requeue and resume).

- **Config change**: edit `config/`, open a PR (CI validates it against the pinned version), merge.
- **Turn a service on/off**: flip `enabled` in `config/services/<name>.yaml` (slack, github, jira, confluence, sentry, aws), PR, merge.
- **Upgrade**: bump the pin in `requirements.txt`, PR, merge. Roll back by reverting the commit.
- **Dashboard edits**: point `dashboard.auto_commit.repo` at this repository so live edits from Mission Control land here as commits.
