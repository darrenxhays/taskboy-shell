...
- [ ] Review `/etc/taskboy/config.yaml` and `/etc/taskboy/services/`. On first install they are seeded from the packaged examples; every merge to this repo's `main` then ships your committed `config/` and `skills/` to the host (section 3c). Point `dashboard.auto_commit.repo` at this repository so live dashboard edits land here as commits and flow back out through the same pipeline.
