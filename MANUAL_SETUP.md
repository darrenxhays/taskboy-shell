...
Start from the fully commented examples that ship inside the package:

```bash
taskboy setup --local    # seeds config/config.yaml + config/services/*.yaml (+ task_started_messages.yaml) from the packaged examples, nothing else
```

(Equivalently: `taskboy assets templates .` copies the packaged `templates/` tree — the config example, per-service config examples, personality examples, conventions template, skill templates, and Slack manifest — into the current directory, and you copy files from there by hand.)
...
The wizard just copies packaged templates; do the same:

```bash
taskboy assets templates /tmp/seed
cp /tmp/seed/templates/conventions.md config/conventions.md                      # fill in your org's rules
cp /tmp/seed/templates/personality_agent.example.md config/personality_agent.md
cp /tmp/seed/templates/personality_reviewer.example.md config/personality_reviewer.md   # only if the reviewer is enabled
```
...
`skills/` starts empty; the workflow templates ship inside the package. To install one by hand, copy the template directory into `skills/` and replace **every** `{{variable}}` in the copied `SKILL.md` — the loader does no substitution at runtime and rejects leftovers:

```bash
taskboy assets templates /tmp/seed
cp -r /tmp/seed/templates/skills/review skills/review
# then edit skills/review/SKILL.md and fill in {{agent_name}}, {{repo_list}}, ...
```

The variable table, frontmatter reference, and the `requires` rule (any skill listed in another's `requires` must be installed too) are in the extracted `templates/skills/README.md`. Integration constraints: `jira2pr` and `slack2jira` need Jira configured; `release` and `discoverissues` need `github.self_repo`.
